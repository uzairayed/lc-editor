...
2. **Understand long process clips (optional).** For detailing / album footage, call `media_understand` (default process roles) to get cheap span cards `{media_id, in_s, out_s, role_hint, score, keyframe_path, reason}` without a full-video VLM. Budget defaults to ~48 candidate keyframes per media (max 64). If a span is uncertain, `media_understand_refine(media_id, in_s, out_s)` densely re-samples only that window. Results stamp `understand:{role}` on the shot index so later `shots_rank` / `shots_search` prefer those spans.
