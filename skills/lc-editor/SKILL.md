...
2. **Understand long process clips (optional).** For detailing / album footage, call `media_understand` (default process roles) to get cheap span cards `{media_id, in_s, out_s, role_hint, score, keyframe_path, reason}` without a full-video VLM. Default selection is adaptive (FOCUS coarse→fine; AKS relevance+coverage when you pass a concrete `query`). Budget defaults to ~48 candidate keyframes per media (max 64). For a whole album under one pool, pass `shared_budget=true`. Response `metrics` reports `frames_scored / duration_s` vs a uniform 1fps baseline. If a span is uncertain, `media_understand_refine(media_id, in_s, out_s)` densely re-samples only that window. Results stamp `understand:{role}` on the shot index so later `shots_rank` / `shots_search` prefer those spans.
...
**Media:** `import_file`, `import_folder`, `media_list`, `media_tag`, `probe`, `media_analyze`, `media_understand`, `media_understand_refine`, `shots_list`, `shots_search`, `shots_rank`
