...
2. **Understand long process clips (Director first).** For detailing / album footage, call `media_understand` (default process roles) to get cheap span cards `{media_id, in_s, out_s, role_hint, score, keyframe_path, reason, role_scores}` without a full-video VLM. Default selection is adaptive (FOCUS coarse→fine; AKS relevance+coverage when you pass a concrete `query`). Budget defaults to ~48 candidate keyframes per media (max 64). For a whole album under one pool, pass `shared_budget=true`. Response `metrics` reports `frames_scored / duration_s` vs a uniform 1fps baseline. If a span is uncertain, `media_understand_refine(media_id, in_s, out_s)` densely re-samples only that window. When a high-value span looks spatially busy (low subject dominance), call `media_understand_spatial` (or `media_understand_refine(..., spatial=true)`) for denser keyframes plus soft `focus_hint` (`focus_x`/`focus_y`); suggestions warn only and never block export. Results stamp `understand:{role}` on the shot index so later `shots_rank` / `shots_search` strongly prefer those spans. Role hints should diversify across `before` / wash-work / `interior` / `wheel` / `after` on silent detailing (not wash/skip_face only): soft priors from `media_tag`, filename tokens, album order, and temporal thirds, plus an album diversify pass. Then call `understand_timeline` for a role-labeled beat list (`before` → `wash` → … → `after` → `hero`) before picking clips. Optional `highlights_suggest(target_s, style="process"|"reel")` ranks candidate beat sheets packed to a duration (process ~60s should yield a complete before→process→after arc near 45–70s when spans exist), preferring transformation arcs over virality/transcript hacks; it is suggest-only (no auto-export; you still lock the story). Do **not** open a separate frame-watching bot; Director uses these span cards.

3. **Card, then rank, then lock the story.** Call `label_queue` / `label_get` (or `media_card_propose`) and confirm every `needs_confirmation` with `media_card_confirm` or `shot_card_confirm` (day, before/after/process, what is in frame) before story lock. `label_readiness.ready` should be true; `labels_bulk_confirm` / `labels_clear` / `labels_undo` are the same writes the labeling GUI uses. `shoot_day_suggest` clusters capture times; folder names (`before/`, `day1`) already tag untagged imports. For multi-day albums (detailing, travel), confirmed cards keep Day 1 dusty → wash → machine → after inside LC. Prefer `understand_timeline` / `highlights_suggest` beats, then `shots_rank(role=…)` / `shots_search` to pick shots without opening every file. Narrative roles: `hook`, `journey`, `site_wide`, `site_detail`, `closer`. Process roles: `before`, `wash`, `detail`, `after`, `hero`, `skip_face`, `engine`, `wheel`, `interior`, `machine`. Ranked rows include `media_id`, `in_s` / `out_s`, `score`, `thumb`. Look at keyframes with `thumbnail` or `contact_sheet`. `clip_add` in tagged day/role order.
...
**Media:** `import_file`, `import_folder`, `media_list`, `media_tag`, `shoot_day_suggest`, `media_card_propose`, `media_card_confirm`, `label_queue`, `label_get`, `shot_card_confirm`, `labels_bulk_confirm`, `labels_clear`, `labels_undo`, `label_conflicts`, `label_readiness`, `probe`, `media_analyze`, `media_understand`, `media_understand_refine`, `media_understand_spatial`, `understand_timeline`, `highlights_suggest`, `shots_list`, `shots_search`, `shots_rank`
...
**SFX:** `sfx_list`, `sfx_import` / `sfx_pack_add` (CC0 / Mixkit / Pixabay wav/mp3 into project `user-sfx/` + `ATTRIBUTION.json`), `sfx_place`, `sfx_caption_auto`, `sfx_transition_auto`, `sfx_zoom_auto`
