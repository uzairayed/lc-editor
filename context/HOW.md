...
2. **Album lock.** Call `media_card_propose`. Confirm every `needs_confirmation` with `media_card_confirm` after you actually look at the keyframes (day, before / after / process, what is in frame). `media_list.card_coverage` should read carded for every visual file you will use. Do not story-lock from uncarded bookends. Optional: `shoot_day_suggest` then confirm.
3. Open `context/INDEX.md`. Collect every card whose trigger matches a named subject. Read those files. Skip the rest.
4. Resolve overlaps on one clip: action beats object, reveal beats passing, the longer hold wins. Then re-budget the reel so the sum still lands in 15 to 28s. A 15 to 28s cut gets **one** reveal or **one** vista as its long hold, not both at the top of their ranges. A 30s owner experiment may sit in the 28 to 60s warning band.
5. Choose a layout card, not a generic stack:
...
6. Set `in_s` / `out_s` from the card's cut rule, duration from the hold word, role from the card, caption and motion from the card. Stretch the clip to caption hold if a caption is on it (`clip_fit`).
7. Done: every kept shot has either a card decision written next to it, or an explicit "no card, **ack**". A layout counts as one decision. The timeline duration is inside 15 to 28s, or you have already told the owner why it must sit in the 28 to 60s warning band.
